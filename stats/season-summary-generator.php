<?php
    ...
    const MATCH_TYPE_QUERY_MAP = array(
        "League" => "m.CompetitionType = 'League'",
        "Friendly" => "m.CompetitionType = 'Friendly'",
        "Tour" => "m.CompetitionType = 'Tour'"
    );
    ...
    class SeasonSummaryGenerator
    {
        ...
        {
            $this->_db = $db;
            $this->_aggregator = new SummaryAggregator($db, PERIOD_SEASON, PERIOD_SEASON, PERIOD_SEASON);
        }
        ...
        public function generate_summaries($season)
        {
            log\info("      Matches");
            $this->generate_matches_summaries($season);
            log\info("      Batting");
            $this->generate_batting_summaries($season);
            log\info("      Bowling");
            $this->generate_bowling_summaries($season);
            log\info("      Fielding");
            $this->generate_fielding_summaries($season);
            log\info("      Aggregations");
            log\info("        League + Friendly => Regular");
            $this->_aggregator->aggregate_summaries($season, "League", $season, "Friendly", $season, "Regular");
        }

        private function generate_matches_summaries($season)
        {
            $db = $this->_db;
            ...
            foreach (MATCH_TYPE_QUERY_MAP as $match_type => $match_type_query)
            {
                log\info("        {$match_type}");
                foreach ($players as $player_name => $player)
                {
                    ...
                    $insert_matches_summary->bindValue(":MatchType", $match_type);
        ...
        private function generate_batting_summaries($season)
        {
            $db = $this->_db;
            $players = get_players_by_name($db);
            ...
            foreach (MATCH_TYPE_QUERY_MAP as $match_type => $match_type_query)
            {
                ...
                foreach ($players as $player_name => $player)
                {
                    ...
                    $result = $statement->execute()->fetchArray(SQLITE3_ASSOC);
                    db_bind_values_from_row($insert_batting_summary, $result);
                    $insert_batting_summary->bindValue(":MatchType", $match_type);
        ...
        private function generate_bowling_summaries($season)
        {
            $db = $this->_db;
            $players = get_players_by_name($db);
            ...
            foreach (MATCH_TYPE_QUERY_MAP as $match_type => $match_type_query)
            {
                ...
                foreach ($players as $player_name => $player)
                {
                    ...
                    $result = $statement->execute()->fetchArray(SQLITE3_ASSOC);
                    db_bind_values_from_row($insert_bowling_summary, $result);
                    $insert_bowling_summary->bindValue(":MatchType", $match_type);
        ...
        private function generate_fielding_summaries($season)
        {
            $db = $this->_db;
            $players = get_players_by_name($db);
            ...
            foreach (MATCH_TYPE_QUERY_MAP as $match_type => $match_type_query)
            {
                ...
                foreach ($players as $player)
                {
                    ...
                    $insert_fielding_summary->bindValue(":MatchType", $match_type);
